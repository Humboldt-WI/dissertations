...
def plot_cost(training, validation, name, model, epochs, best_epoch):
    ...
    plt.savefig('D:\\DeepCreditSurv\\DeepCreditSurv\\models\\DATE\\plots\\{}_{}'.format(model, name))
...
def box_plots(empirical, predicted, name='data', time='days', log_domain=True):
    ...
    plt.savefig('D:\\DeepCreditSurv\\DeepCreditSurv\\models\\DATE\\plots\\{}_box_plot'.format(name))
...
def hist_plots(samples, name, xlabel, empirical=None):
    ...
    plt.xlabel(xlabel)
    plt.savefig("D:\\DeepCreditSurv\\DeepCreditSurv\\models\\DATE\\plots\\{}_hist".format(name))
    plt.figure()
