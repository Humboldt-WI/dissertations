...
class Cox_Time:
    ...
    def find_lr(self):
        ...
        model = CoxTime(net, tt.optim.Adam, labtrans=self.labtrans)

        batch_size = 256
        lr_finder = model.lr_finder(self.x_train, self.y_train, batch_size, tolerance=2)
        _ = lr_finder.plot()
    ...
    def build_model(self, lr=0.01):
        ...
        val = tt.tuplefy(self.x_val, self.y_val)
        batch_size = 256
        model = CoxTime(net, tt.optim.Adam, labtrans=self.labtrans)
