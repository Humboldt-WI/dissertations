"""This file defines the dynamic etm object and was obtained from https://github.com/adjidieng/DETM/blob/master/detm.py, 10/2020. Some changes were made to the original file."""
...
class DETM(nn.Module):
    def __init__(self, args, embeddings):
        ...
        self.theta_hidden_size = args.theta_hidden_size
        self.eta_hidden_size = args.eta_hidden_size
        self.rho_size = args.rho_size
        self.eta_nlayers = args.eta_nlayers
        self.theta_dropout_value = args.theta_dropout
        self.theta_dropout = nn.Dropout(args.theta_dropout)
        self.delta = args.delta
        self.train_embeddings = args.train_embeddings
        self.theta_act, args.theta_act = self.get_activation(args.theta_act)
        ...
        self.q_theta = nn.Sequential(
                    nn.Linear(args.vocab_size+args.num_topics, args.theta_hidden_size), 
                    self.theta_act,
                    nn.Linear(args.theta_hidden_size, args.theta_hidden_size),
                    self.theta_act,
                )
        self.mu_q_theta = nn.Linear(args.theta_hidden_size, args.num_topics, bias=True)
        self.logsigma_q_theta = nn.Linear(args.theta_hidden_size, args.num_topics, bias=True)
    ...
    def reparameterize(self, mu, logvar):
        """Returns a sample from a Gaussian distribution via reparameterisation.
        """
        if self.training:
    ...
    def get_eta(self, rnn_inp):
        ...
        for t in range(1, self.num_times):
            # concatenate the hidden size with eta from the previous period:
    ...
    def get_theta(self, eta, bows, times): ## amortized inference
        ...
        if self.theta_dropout_value > 0:
            q_theta = self.theta_dropout(q_theta)
    ...
    def init_hidden(self):
        """Initializes the first hidden state of the RNN inference network for \eta.
        """
